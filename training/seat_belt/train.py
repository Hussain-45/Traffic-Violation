"""
Seat Belt Model Training Script
==============================
Provides a reusable training pipeline for custom YOLOv8 seat belt / driver distraction models.
...
Usage:
  python training/seat_belt/train.py --epochs 50 --batch 16 --model-size n --device cpu
"""
import os
import sys
import argparse
...
TRAINING_DIR = PROJECT_ROOT / "training" / "seat_belt"
DATASET_YAML = PROJECT_ROOT / "datasets" / "detection" / "seat_belt" / "data.yaml"
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Train custom YOLOv8 model for Seat Belt Detection.")
    ...
    parser.add_argument("--resume", action="store_true", help="Resume training from training/seat_belt/checkpoints/last.pt")
...
def run_training():
    ...
    if not DATASET_YAML.exists():
        ...
        logger.error("Verify datasets/detection/seat_belt directory is populated.")
        sys.exit(1)
    ...
    # Start training
    logger.info("Starting Ultralytics training run for Seat Belt...")
    results = model.train(
    ...
    best_weights = TRAINING_DIR / "runs" / "seatbelt_run" / "weights" / "best.pt"
    last_weights = TRAINING_DIR / "runs" / "seatbelt_run" / "weights" / "last.pt"
    
    if best_weights.exists():
        # Save to training/seat_belt/weights
        dest_best = TRAINING_DIR / "weights" / f"seatbelt_{args.model_size}_best.pt"
        ...
        central_best = OUTPUT_BEST_DIR / "seatbelt_best.pt"
