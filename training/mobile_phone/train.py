"""
Mobile Phone Model Training Script
=================================
Provides a reusable training pipeline for custom YOLOv8 mobile phone / driver behavior models.
...
Usage:
  python training/mobile_phone/train.py --epochs 50 --batch 16 --model-size n --device cpu
"""
import os
import sys
import argparse
...
TRAINING_DIR = PROJECT_ROOT / "training" / "mobile_phone"
DATASET_YAML = PROJECT_ROOT / "datasets" / "classification" / "driver_behavior" / "data.yaml"
...
def setup_training_directories():
    ...
    folders = ["configs", "weights", "runs", "results", "logs", "checkpoints", "exports"]
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Train custom YOLOv8 model for Mobile Phone Detection.")
...
def run_training():
    ...
    if not DATASET_YAML.exists():
        ...
        logger.error("Verify datasets/classification/driver_behavior directory is populated.")
        sys.exit(1)
    ...
    # Start training
    logger.info("Starting Ultralytics training run for Mobile Phone...")
    results = model.train(
    ...
    best_weights = TRAINING_DIR / "runs" / "mobile_run" / "weights" / "best.pt"
    last_weights = TRAINING_DIR / "runs" / "mobile_run" / "weights" / "last.pt"
    
    if best_weights.exists():
        # Save to training/mobile_phone/weights
        dest_best = TRAINING_DIR / "weights" / f"mobile_{args.model_size}_best.pt"
        ...
        central_best = OUTPUT_BEST_DIR / "mobile_best.pt"
