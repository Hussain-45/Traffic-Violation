...
======================================
Provides a reusable training pipeline for custom YOLOv8 helmet models.
...
Supports:
  ...
  4. Dynamic model size selection (nano, small, medium, large, x-large)

Usage:
  python training/helmet/train.py --epochs 50 --batch 16 --model-size n --device cpu
"""
import os
import sys
import argparse
...
TRAINING_DIR = PROJECT_ROOT / "training" / "helmet"
DATASET_YAML = PROJECT_ROOT / "datasets" / "detection" / "helmet" / "data.yaml"
...
def setup_training_directories():
    ...
    folders = ["configs", "weights", "runs", "results", "logs", "checkpoints"]
...
def parse_arguments():
    parser = argparse.ArgumentParser(description="Train custom YOLOv8 model for Helmet Detection.")
    ...
    parser.add_argument("--resume", action="store_true", help="Resume training from training/helmet/checkpoints/last.pt")
...
def run_training():
    ...
    if not DATASET_YAML.exists():
        ...
        logger.error("Verify datasets/detection/helmet directory is populated.")
        sys.exit(1)
        
    logger.info(f"Using dataset configuration: {DATASET_YAML}")
    
    # 2. Check for dynamic model loading
    # Resolve initial model path
    ...
    # Start training
    # Note: We save runs to training/helmet/runs and checkpoints to checkpoints/
    # We use ultralytics built-in callbacks or arguments
    logger.info("Starting Ultralytics training run...")
    results = model.train(
    ...
    best_weights = TRAINING_DIR / "runs" / "helmet_run" / "weights" / "best.pt"
    last_weights = TRAINING_DIR / "runs" / "helmet_run" / "weights" / "last.pt"
    
    if best_weights.exists():
        # Save to training/helmet/weights
        dest_best = TRAINING_DIR / "weights" / f"helmet_{args.model_size}_best.pt"
        ...
        central_best = OUTPUT_BEST_DIR / "helmet_best.pt"
